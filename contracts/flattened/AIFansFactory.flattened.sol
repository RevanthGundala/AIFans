...
pragma solidity >=0.8.0 ^0.8.20 ^0.8.23;
...
contract FanMedia is ERC721URIStorage {
    uint256 public tokenId;

    constructor(string memory name, string memory symbol) ERC721(name, symbol) {}

    function mint(string memory blob, address to) public returns (uint256 _tokenId) {
        _tokenId = tokenId;
        _mint(to, _tokenId);
        _setTokenURI(_tokenId, blob);
        tokenId++;
